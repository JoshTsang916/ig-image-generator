...
export async function uploadToCloudinary(imageBuffer, publicId, cloudName, uploadPreset) {
    ...
    // 確保 publicId 不含斜線和特殊字元
    const safePublicId = publicId.replace(/[^a-zA-Z0-9_-]/g, '_');

    console.log(`[Cloudinary] Uploading to ${cloudName} with preset ${uploadPreset}, id: ${safePublicId}`);
