...
export async function renderSlides(template, backgroundUrl, slides) {
    ...
    // 讀取 Logo 並轉 Base64 (避免路徑問題)
    let logoDataUrl = '';
    try {
        const logoPath = path.join(TEMPLATES_DIR, 'assets', 'TheLifeForgeLogo.png');
    ...
    for (let i = 0; i < slides.length; i++) {
        ...
        const slideData = {
            ...
            backgroundUrl,
            coverTitle,
            logoDataUrl // 傳入 Logo Base64 string
        };
...
function getTemplateFile(template, slideType) {
    const mapping = {
        ...
        quote: {
            ...
            'single': 'quote/single.html',
            'cta': 'quote/cta.html'
        },
        journal: {
            'single': 'journal/single.html',
            'cta': 'journal/cta.html'
        }
    };
...
async function buildHtml(templatePath, data) {
    ...
    html = html.replace(/\{\{(\w+)\}\}/g, (match, key) => {
        if (key in data) {
            ...
            if (typeof value === 'string') {
                // 對 journal 內容，把括號轉成斜體 span（移除括號）
                if (key === 'content') {
                    value = value.replace(/[（(](.+?)[）)]/g, '<span class="note">$1</span>');
                }
                // 處理換行符號
                value = value.replace(/\n/g, '<br>');
            }
    ...
    // 如果有 Logo Base64，替換所有 TheLifeForgeLogo.png 的引用
    if (data.logoDataUrl) {
        html = html.replace(/src=["'].*?TheLifeForgeLogo\.png["']/g, `src="${data.logoDataUrl}"`);
    }
